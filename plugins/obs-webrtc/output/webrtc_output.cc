...
void register_webrtc_output(){
    ...
    info.create = [](obs_data_t *settings,obs_output_t *output) -> void*{
        return rtc::make_ref_counted<RTC::WHIPOutput>(settings,output).release();
    };
    info.destroy = [](void *p){
...
WHIPOutput::WHIPOutput(obs_data_t *,obs_output_t *output) : signal_thread_(rtc::Thread::CreateWithSocketServer()),output_(output)
{
    video_capture_ = std::make_unique<OBSVideoCapture>();
    video_track_source_ = rtc::make_ref_counted<OBSTrackSource>(video_capture_.get()); 
...
bool WHIPOutput::Start(){
    ...
    do{
        if(flag)
          return true;
    }while(!start_.compare_exchange_weak(flag,true,std::memory_order_acq_rel));
    ...
    signal_thread_->PostTask([this](){
        ...
        auto res = pc_factory_->CreatePeerConnectionOrError(config,std::move(pc_dependencies));
        RTC_CHECK(res.ok());
...
void WHIPOutput::AddTracks(){
    video_track_ = pc_factory_->CreateVideoTrack(video_track_source_,"video_label");
    RTC_CHECK(video_track_ != nullptr) << "create video track failed!";

    auto result = pc_->AddTrack(video_track_,{"stream"});
    RTC_CHECK(result.ok()) << "failed to add video track to pc: " << result.error().message();
    ...
    RTC_CHECK(result.ok()) << "failed to add audio track to pc: " << result.error().message();
...
void WHIPOutput::OnSuccess(webrtc::SessionDescriptionInterface* desc){
    pc_->SetLocalDescription(DummySetSessionDescriptionObserver::Create().get(), desc);
    std::string sdp;
    desc->ToString(&sdp);
    RTC_LOG(LS_INFO) << "local sdp: " << sdp;
    
    if(ConnectServer(sdp)){
        RTC_CHECK(obs_output_begin_data_capture(output_,0)) << "obs capture failed";
    }
}
...
bool WHIPOutput::ConnectServer(const std::string &sdp){
	...
	if (res != CURLE_OK) {
		RTC_LOG(LS_INFO) << "Connect failed: CURL returned result not CURLE_OK";
	...
	if (response_code != 201) {
	    RTC_LOG(LS_INFO) << "Connect failed: HTTP endpoint returned response code" 
                         << response_code;
	...
	if (read_buffer.empty()) {
		RTC_LOG(LS_INFO) << "Connect failed: No data returned from HTTP endpoint request";
		obs_output_signal_stop(output_, OBS_OUTPUT_CONNECT_FAILED);
		return false;
	}
	...
	if (location_headers.size() < static_cast<size_t>(redirect_count) + 1) {
        RTC_LOG(LS_INFO) << "WHIP server did not provide a resource URL via the Location header";
		obs_output_signal_stop(output_, OBS_OUTPUT_CONNECT_FAILED);
		return false;
	}
	...
	if (rc) {
		RTC_LOG(LS_INFO) << "WHIP server provided a invalid resource URL via the Location header";
		obs_output_signal_stop(output_, OBS_OUTPUT_CONNECT_FAILED);
		return false;
	}

    webrtc::SdpParseError error;
    auto remote_sdp = webrtc::CreateSessionDescription(webrtc::SdpType::kAnswer,read_buffer,&error);
    ...
    pc_->SetRemoteDescription(DummySetSessionDescriptionObserver::Create().get(),
         remote_sdp.release()
        );    
...
void WHIPOutput::OnIceConnectionChange(
    ...
    RTC_LOG(LS_INFO) << "IceState: " << webrtc::PeerConnectionInterface::AsString(state);
}
    
void WHIPOutput::OnIceGatheringChange(
        webrtc::PeerConnectionInterface::IceGatheringState state){
    RTC_LOG(LS_INFO) << "IceGatheringState: " << webrtc::PeerConnectionInterface::AsString(state);
}
